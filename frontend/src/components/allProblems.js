...
const AllProblems = ({
  ...
  const changeStatus = async () => {
    const response = await fetch(
      "/api/problem/problemStatus/?id=" + id + "&status=" + buttonName,
      { method: "PATCH" }
    );
  ...
  const unresStatus = async () => {
    const response = await fetch(
      "/api/problem/problemStatus/?id=" + id + "&status=" + unresButton,
      { method: "PATCH" }
    );
    if (response.status == 200) {
      handleShow();
  ...
  return (
    <div className="">
      <Card>
        ...
        <Card.Header>
          <Nav variant="tabs" defaultActiveKey="#first">
            <Nav.Item>
              <Nav.Link href="#first" onClick={bb1}>
                <p>content</p>
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link href="#second" onClick={bb2}>
                <p>details</p>
              </Nav.Link>
            </Nav.Item>
            <Nav.Item style={{ display: showTab }}>
              <Nav.Link href="#third" onClick={bb3}>
                <p style={{ color: "firebrick" }}>
                  ...
                  <i
                    class="gg-bell"
                    style={{ position: "relative", left: 70, bottom: 12 }}
                  ></i>{" "}
                </p>
              </Nav.Link>
            </Nav.Item>
        ...
        <Card.Body style={{ display: b1 }}>
          ...
          <div style={{ display: "flex" }}>
            ...
            <Button
              variant="danger"
              style={{ display: unres, marginLeft: 20 }}
              onClick={unresStatus}
            >
              mark as Resolved
            </Button>
      ...
      <Modal
        ...
        keyboard={false}
      >
        <Modal.Body>marked to {buttonName} successfully.</Modal.Body>
        <Modal.Footer>
